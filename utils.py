...
def show_prob_mush_mlp(probs):
    
    labels = ['Edible', 'Poisonous']

    probs_np = probs.data.squeeze().cpu().numpy()
    
    ft = 15
    y_pos = np.arange(len(probs_np))*1.2
    target=2
    width=0.9
    col= 'blue'
    ...
    # the plot
    ax.barh(y_pos, probs_np, width , align='center', color=col)
    ax.set_xlim([0, 1.3])

    # y label
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels, fontsize=ft)
    ax.invert_yaxis()  
    ...
    for i in range(len(probs_np)):
            str_nb="{0:.3f}".format(probs_np[i])
            ax.text( probs_np[i] + 0.05 , y_pos[i] ,str_nb ,
                    horizontalalignment='left', verticalalignment='center',
                    transform=ax.transData, color= col,fontsize=ft)
    plt.show()
